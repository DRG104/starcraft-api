...
// pull in Mongoose model for units
...
// { unit: { title: '', text: 'foo' } } -> { unit: { text: 'foo' } }
...
router.get('/units', (req, res, next) => {
	Unit.find()
		.populate('owner')
		.then((units) => {
			// `units` will be an array of Mongoose documents
			// we want to convert each one to a POJO, so we use `.map` to
			// apply `.toObject` to each one
			return units.map((unit) => unit.toObject())
		})
		// respond with status 200 and JSON of the units
...
// SHOW
// GET /units/5a7db6c74d55bc51bdf39793
...
router.post('/units', requireToken, (req, res, next) => {
	// set owner of new unit to be current user
	req.body.unit.owner = req.user.id

	Unit.create(req.body.unit)
		// respond to succesful `create` with status 201 and JSON of new "unit"
		.then((unit) => {
...
// UPDATE
// PATCH /units/5a7db6c74d55bc51bdf39793
...
// DESTROY
// DELETE /units/5a7db6c74d55bc51bdf39793
router.delete('/units/:id', requireToken, (req, res, next) => {
	Unit.findById(req.params.id)
		.then(handle404)
		.then((unit) => {
			// throw an error if current user doesn't own `unit`
			requireOwnership(req, unit)
			// delete the unit ONLY IF the above didn't throw
			unit.deleteOne()
		})
